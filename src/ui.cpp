...
Button::Button(float x, float y, float width, float height, const std::string& txt, Color col)
    : rect{x, y, width, height}, text(txt), color(col), isToggle(false), isToggled(false) {}
...
void Button::draw() {
    ...
    // Center text
    Vector2 textSize = MeasureTextEx(GetFontDefault(), text.c_str(), FONT_SIZE, 1);
    float textX = rect.x + (rect.width - textSize.x) / 2;
    float textY = rect.y + (rect.height - textSize.y) / 2;
    
    DrawTextEx(GetFontDefault(), text.c_str(), {textX, textY}, FONT_SIZE, 1, textColor);
}

// InputField implementation
InputField::InputField(float x, float y, float width, float height, const std::string& lbl, int maxLen)
    : rect{x, y, width, height}, label(lbl), text(""), isActive(false), maxLength(maxLen) {}
...
void InputField::draw() {
    ...
    DrawTextEx(GetFontDefault(), label.c_str(), {rect.x, rect.y - 20}, FONT_SIZE, 1, COLOR_TEXT);
    
    // Draw text
    DrawTextEx(GetFontDefault(), text.c_str(), {rect.x + 5, rect.y + (rect.height - FONT_SIZE) / 2}, FONT_SIZE, 1, COLOR_TEXT);
    
    // Draw cursor if active
    if (isActive) {
        float textWidth = MeasureTextEx(GetFontDefault(), text.c_str(), FONT_SIZE, 1).x;
        DrawRectangle(rect.x + 5 + textWidth, rect.y + (rect.height - FONT_SIZE) / 2, 2, FONT_SIZE, COLOR_TEXT);
    }
}
...
Dropdown::Dropdown(float x, float y, float width, float height, const std::string& lbl, const std::vector<std::string>& opts)
    : rect{x, y, width, height}, label(lbl), options(opts), selectedIndex(0), isOpen(false) {}
...
void Dropdown::draw() {
    ...
    DrawTextEx(GetFontDefault(), displayText.c_str(), {rect.x + 5, rect.y + (rect.height - FONT_SIZE) / 2}, 
    ...
    if (isOpen) {
        for (int i = 0; i < options.size(); i++) {
            ...
            Color optTextColor = (i == selectedIndex) ? COLOR_LIGHT_TEXT : COLOR_TEXT;
            
            if (CheckCollisionPointRec(GetMousePosition(), optionRect)) {
                optBgColor = (i == selectedIndex) ? ColorBrightness(COLOR_PRIMARY, 0.2f) : COLOR_HIGHLIGHT;
            }
            
            DrawRectangleRec(optionRect, optBgColor);
            DrawRectangleLinesEx(optionRect, 1, COLOR_BORDER);
            DrawTextEx(GetFontDefault(), options[i].c_str(), {optionRect.x + 5, optionRect.y + (optionRect.height - FONT_SIZE) / 2}, 
...
void ScheduleUI::initialize() {
    // Create tab buttons
    float tabWidth = SCREEN_WIDTH / 5.0f;
    tabButtons.push_back(Button(0, 0, tabWidth, 40, "Courses", COLOR_PRIMARY));
    tabButtons.push_back(Button(tabWidth, 0, tabWidth, 40, "Teachers", COLOR_PRIMARY));
    tabButtons.push_back(Button(tabWidth * 2, 0, tabWidth, 40, "Sections", COLOR_PRIMARY));
    tabButtons.push_back(Button(tabWidth * 3, 0, tabWidth, 40, "Schedule", COLOR_PRIMARY));
    tabButtons.push_back(Button(tabWidth * 4, 0, tabWidth, 40, "Preferences", COLOR_PRIMARY));
    ...
    tabButtons[4].isToggle = true;
    
    // Create action buttons
    actionButtons.push_back(Button(SCREEN_WIDTH - 200, SCREEN_HEIGHT - 50, 180, 40, "Generate Schedule", COLOR_ACCENT));
    
    // Initialize course tab components
    courseInputs.push_back(InputField(PADDING, 80, 300, 40, "Course Code (e.g., CS101)"));
    ...
    addCourseButton = Button(PADDING, 290, 300, 40, "Add Course", COLOR_PRIMARY);
    
    // Initialize teacher tab components
    teacherInputs.push_back(InputField(PADDING, 80, 300, 40, "Teacher ID"));
    teacherInputs.push_back(InputField(PADDING, 150, 300, 40, "Teacher Name"));
    addTeacherButton = Button(PADDING, 290, 300, 40, "Add Teacher", COLOR_PRIMARY);
    
    // Initialize section tab components
    sectionInputs.push_back(InputField(PADDING, 80, 300, 40, "Section ID (e.g., A, B, C)"));
    ...
    addSectionButton = Button(PADDING, 290, 300, 40, "Add Section", COLOR_PRIMARY);
    
    // Initialize preference tab components
    std::vector<std::string> prefTypes = {"Prefer Teacher", "Prefer Time Slot", "Avoid Teacher", "Avoid Time Slot"};
    preferenceTypeDropdown = Dropdown(PADDING, 80, 300, 40, "Preference Type", prefTypes);
    preferenceCourseDropdown = Dropdown(PADDING, 150, 300, 40, "Select Course", {});
    preferenceTeacherDropdown = Dropdown(PADDING, 220, 300, 40, "Select Teacher", {});
    ...
    for (int day = 0; day < 5; day++) {
        for (int hour = 8; hour <= 17; hour++) {
            ...
            std::string label = TimeSlot::dayToString(day).substr(0, 3) + " " + std::to_string(hour);
            preferenceTimeButtons.push_back(Button(x, y, 55, 25, label, COLOR_BORDER));
            preferenceTimeButtons.back().isToggle = true;
        }
    }
    
    addPreferenceButton = Button(PADDING, 600, 300, 40, "Add Preference", COLOR_PRIMARY);
...
void ScheduleUI::update() {
    ...
    switch (currentTab) {
        ...
        case Tab::SECTIONS:
            ...
            for (auto& input : sectionInputs) {
                input.update();
            }
            
            // Update dropdowns
            courseDropdown.update();
            teacherDropdown.update();
            
            // Check if Add Section button is clicked
            if (addSectionButton.isClicked()) {
                addSection();
            }
            break;
        ...
        case Tab::PREFERENCES:
            ...
            // Update time slot buttons only if time slot preference is selected
            if (preferenceTypeDropdown.getSelectedIndex() == 1 || preferenceTypeDropdown.getSelectedIndex() == 3) {
...
void ScheduleUI::drawCoursesTab() {
    // Draw heading
    DrawTextEx(GetFontDefault(), "Courses", {PADDING, 50}, HEADER_FONT_SIZE, 1, COLOR_TEXT);
    ...
    DrawTextEx(GetFontDefault(), "Existing Courses:", {PADDING, 350}, FONT_SIZE, 1, COLOR_TEXT);
    
    int y = 380;
    for (const auto& course : scheduler->getCourses()) {
        std::stringstream ss;
        ss << course->code << ": " << course->title << " (" << course->creditHours << " credits)";
        
        DrawTextEx(GetFontDefault(), ss.str().c_str(), {PADDING, (float)y}, FONT_SIZE, 1, COLOR_TEXT);
        y += FONT_SIZE + 5;
    }
}
...
void ScheduleUI::drawTeachersTab() {
    // Draw heading
    DrawTextEx(GetFontDefault(), "Teachers", {PADDING, 50}, HEADER_FONT_SIZE, 1, COLOR_TEXT);
    ...
    DrawTextEx(GetFontDefault(), "Existing Teachers:", {PADDING, 350}, FONT_SIZE, 1, COLOR_TEXT);
...
void ScheduleUI::drawSectionsTab() {
    // Draw heading
    DrawTextEx(GetFontDefault(), "Sections", {PADDING, 50}, HEADER_FONT_SIZE, 1, COLOR_TEXT);
    ...
    // Draw Add Section button
    addSectionButton.draw();
    
    // Draw list of existing sections
    DrawTextEx(GetFontDefault(), "Existing Sections:", {PADDING, 350}, FONT_SIZE, 1, COLOR_TEXT);
    ...
    for (const auto& section : scheduler->getSections()) {
        ...
        if (section->teacher) {
            ss << " - Teacher: " << section->teacher->name;
        }
...
void ScheduleUI::drawScheduleTab() {
    // Draw heading
    DrawTextEx(GetFontDefault(), "Class Schedule", {PADDING, 50}, HEADER_FONT_SIZE, 1, COLOR_TEXT);
...
void ScheduleUI::drawPreferencesTab() {
    ...
    if (preferenceTypeDropdown.getSelectedIndex() == 0 || preferenceTypeDropdown.getSelectedIndex() == 2) {
        // Teacher preference
        preferenceTeacherDropdown.draw();
    } else if (preferenceTypeDropdown.getSelectedIndex() == 1 || preferenceTypeDropdown.getSelectedIndex() == 3) {
        // Time slot preference
        DrawTextEx(GetFontDefault(), "Select Time Slot:", {PADDING, 290 - 20}, FONT_SIZE, 1, COLOR_TEXT);
...
void ScheduleUI::drawScheduleGrid() {
    ...
    for (int hour = 8; hour <= 17; hour++) {
        ...
        DrawRectangleLines(0, y, DAY_HEADER_WIDTH, CELL_HEIGHT, COLOR_BORDER);
        
        // Center text in cell
        Vector2 textSize = MeasureTextEx(GetFontDefault(), timeLabel.c_str(), FONT_SIZE, 1);
        float textX = (DAY_HEADER_WIDTH - textSize.x) / 2;
        float textY = y + (CELL_HEIGHT - textSize.y) / 2;
        
        DrawTextEx(GetFontDefault(), timeLabel.c_str(), {textX, textY}, FONT_SIZE, 1, COLOR_TEXT);
    }
    ...
    for (int day = 0; day < 5; day++) {
        ...
        // Center text in cell
        Vector2 textSize = MeasureTextEx(GetFontDefault(), days[day], FONT_SIZE, 1);
        float textX = x + (dayWidth - textSize.x) / 2;
        float textY = (TIME_HEADER_HEIGHT - textSize.y) / 2;
        
        DrawTextEx(GetFontDefault(), days[day], {textX, textY}, FONT_SIZE, 1, COLOR_TEXT);
    }
...
void ScheduleUI::handleScheduleDrag() {
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        if (draggedSection && !draggedSection->timeSlots.empty()) {
            ...
            // Check if this slot is valid (teacher is available, no conflicts)
            bool isValid = true;
...
void ScheduleUI::addSection() {
    ...
    if (!sectionId.empty() && courseIndex >= 0 && courseIndex < scheduler->getCourses().size() && 
        ...
        if (std::find(course->assignedTeachers.begin(), course->assignedTeachers.end(), teacher) == course->assignedTeachers.end()) {
...
void ScheduleUI::addPreference() {
    ...
    if (prefType >= 0 && courseIndex >= 0 && courseIndex < scheduler->getCourses().size()) {
            ...
            case 2: // Avoid Teacher
                {
                    ...
                    if (teacherIndex >= 0 && teacherIndex < scheduler->getTeachers().size()) {
                        pref.type = (prefType == 0) ? StudentPreference::PREFER_TEACHER : StudentPreference::AVOID_TEACHER;
                        pref.teacherId = scheduler->getTeachers()[teacherIndex]->id;
            ...
            case 3: // Avoid Time Slot
                {
                    ...
                    for (int i = 0; i < preferenceTimeButtons.size(); i++) {
                        if (preferenceTimeButtons[i].isToggled) {
                            ...
                            pref.type = (prefType == 1) ? StudentPreference::PREFER_TIME_SLOT : StudentPreference::AVOID_TIME_SLOT;
...
void ScheduleUI::generateSchedule() {
    ...
    if (success) {
        ...
        tabButtons[2].isToggled = false;
        tabButtons[3].isToggled = true;
        tabButtons[4].isToggled = false;
    }
}

void ScheduleUI::loadDemoData() {
    // Create some courses
    auto cs101 = std::make_shared<Course>("CS101", "Introduction to Programming", 3);
    auto cs201 = std::make_shared<Course>("CS201", "Data Structures", 4);
    auto math101 = std::make_shared<Course>("MATH101", "Calculus I", 3);
